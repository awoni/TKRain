...
function rainSummaryTable(data, url) {
    ...
    for (i = 0; i < data.hr.length; i++) {
        out += '<tr><td>' + data.hr[i].obn + '</td><td>' + data.hr[i].d10_1h_val + '</td><td>' + data.hr[i].d70_10m_val + '</td><td>' + data.hr[i].dt + '</td><td><a href="RainData.html?station=' + data.hr[i].sc + '">リンク</a></td></tr>';
    }
    out += "<table>";
    out += "<p>データ: " + url + "</p>";
